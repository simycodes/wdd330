...
function renderShipList(ships, shipListElement) {
  // I decided to use a table to display my list of ships.
  // The shipList Element is that table and it has 2 children: thead and tbody...we need 
  // to put our ships into tbody...so I reference the second child.
...
async function showShips(url = "https://swapi.dev/api/starships/") {
  ...
  if(results.next) {
    ...
    // being able to hold one listener of the type. In this case that is a good thing 
    // however. Because we are not re-creating the buttons each time we load a new batch 
    // of data we could end up with several listeners attached to each button by the last page. We won't have that issue here.
    next.ontouchend = () => {
      // notice to show the next page we just re-call the showShips function with a new URL
      showShips(data.next); // showShips(results.next); -- why not results.next - verify
    };
  }

  if(results.previous) {
    const prev = document.getElementById("prev");
    // we need to set the once option on the listener since the buttons do not get recreated
    // with each page load. If we don't we will end up with several listeners all trying
    // to fire at once after a few pages
    prev.ontouchend = () => {
