...
// FUNCTIONALITY TO ADD A FAVORITE SCRIPTURE 
...
addScripture.addEventListener("click",() => {
	...
	if(scripture.length == 0 || scripture === undefined) {
		...
		// Variable to help check if favorite scripture added does not exist
		let favoriteScriptureExists = false;
		...
		if(scripturesInLocalStorage === null) {
			...
			// Alert the user if user attempts to add a scripture already added to favorites
			scripturesInLocalStorage.map((scriptureInLoop) => {
				if(scriptureInLoop.content === scripture){
					favoriteScriptureExists = true;
					console.log("Scripture already exists!");
					console.log(favoriteScriptureExists);
					// Create div element to hold a message saying get scripture already exists
					let messageContainer = document.createElement("div");
					let newSpanElement = document.createElement("span");
					newSpanElement.textContent = "Scripture Already Added To Favorites. Add a Different one.";
					messageContainer.appendChild(newSpanElement);
		...
		// Add the scripture to the list of favorite scriptures
		if(favoriteScriptureExists === false){
					// Adding new scripture to the array of scriptures
